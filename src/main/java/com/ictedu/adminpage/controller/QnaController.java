...
public class QnaController {
    ...
    public List<QnaModel> getAllQna() {
        return qnaService.getAllQna();  // 모든 QnA 가져오기
    }
 ...
 // 비밀번호로 보호된 특정 QnA를 ID로 조회
    ...
    public ResponseEntity<QnaModel> getQnaById(
        ...
        if (!qna.getQnaPassword().equals(password)) {
            return ResponseEntity.status(403).build(); // 비밀번호 불일치 시 403 Forbidden
        }
 ...
 // 관리자 또는 사용자 모두 접근 가능한 QnA 상세보기 엔드포인트 (비밀번호 없이)
 ...
 // QnA 답변을 등록하고 상태를 업데이트하는 엔드포인트
    ...
    public ResponseEntity<QnaModel> answerQna(
        ...
        // 변경 사항을 저장
        QnaModel updatedQna = qnaService.updateQna(qnaId, qna);
