...
public class ProofreadSelfService {
    ...
    public String getChatGPTResponse(String text) throws IOException {
     ...
     // 프롬프트 생성
        ...
        promptBuilder.append("비격식적인 표현이나 구어체를 사용하는 경우 수정을 해줘.사용자가 만약 '~했어요'와 같은 '요'로끝나는 구어체로 텍스트를 작성했다면 '~했습니다,~입니다' 와 같이 다로끝나는 문체로 수정해줘. 존댓말이나 격식을 갖춘 문체라면 수정하지 않아도 돼.\n");
        ...
        promptBuilder.append("수정된 텍스트 전체 문장을 보냈다면, 다시 두 줄 띄우고 '▶ 수정 부분은 다음과 같습니다.'로 그 밑에 수정 이유 메세지를 보여줘.\n");
        promptBuilder.append("수정 이유를 보여줄때는 - 하이푼으로 문장을 시작하고 수정 전 문장과 수정된 문장이 반드시 함께 나와야 해.\n");
        promptBuilder.append("하이푼으로 시작하는 수정 이유 문장은 각 문장마다 한줄씩 공백을 줘서 사용자가 보기 쉽게 해줘야해\n");
        ...
        promptBuilder.append("사용자의 원본과 조금이라도 다르게 수정한 부분이 있다면 수정 이유에 모두 포함시켜야해.\n");
